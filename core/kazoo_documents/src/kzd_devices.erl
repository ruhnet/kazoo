...
-export([delay/1, delay/2, set_delay/2]).
...
-export([follow_me/1, follow_me/2, set_follow_me/2]).
...
-spec delay(doc()) -> kz_term:api_pos_integer().
delay(Doc) ->
    delay(Doc, 'undefined').

-spec delay(doc(), Default) -> pos_integer() | Default.
delay(Doc, Default) ->
    kz_json:get_integer_value([<<"delay">>], Doc, Default).

-spec set_delay(doc(), pos_integer()) -> doc().
set_delay(Doc, Delay) ->
    kz_json:set_value([<<"delay">>], Delay, Doc).
...
-spec follow_me(doc()) -> boolean().
follow_me(Doc) ->
    follow_me(Doc, true).

-spec follow_me(doc(), Default) -> boolean() | Default.
follow_me(Doc, Default) ->
    kz_json:get_boolean_value([<<"follow_me">>], Doc, Default).

-spec set_follow_me(doc(), boolean()) -> doc().
set_follow_me(Doc, FollowMe) ->
    kz_json:set_value([<<"follow_me">>], FollowMe, Doc).
...
-spec timeout(doc()) -> kz_term:api_pos_integer().
timeout(Doc) ->
    timeout(Doc, 'undefined').

-spec timeout(doc(), Default) -> pos_integer() | Default.
timeout(Doc, Default) ->
    kz_json:get_integer_value([<<"timeout">>], Doc, Default).

-spec set_timeout(doc(), pos_integer()) -> doc().
